...
import { playlist_tracks, playlists } from '@/drizzle/schema/main';
import { _public, base, isLoggedIn } from '../middleware';

const list = base.use(_public).handler(({ context: { user } }) => {
	if (!user) {
		return [];
	}
	return db.query.playlists.findMany({
		...
		orderBy: [desc(playlists.favorites), desc(playlists.queue), asc(playlists.name)],
	});
});
...
const getById = base
	...
	.handler(async ({ context: { user }, input }) => {
		const playlist = await db.query.playlists.findFirst({
			with: { playlistTracks: { orderBy: playlist_tracks.index } },
...
const insert = base
	...
	.handler(async ({ context: { user }, input: { name, trackIds } }) => {
...
const update = base
	...
	.handler(
		async ({ context: { user }, input: { id: playlistId, name, trackIds } }) => {
			const playlist = await db.query.playlists.findFirst({
...
const dragAndDrop = base
	...
	.handler(
		async ({ context: { user }, input: { playlistId, oldIndex, newIndex } }) => {
			await db.transaction(async (tx) => {
				...
				const otherTrackIndexOne = newIndex > oldIndex ? oldIndex + 1 : oldIndex - 1;
