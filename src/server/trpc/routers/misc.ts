...
import { z } from 'zod';
import { userTable } from '../../../../src/server/drizzle/lucia.js';
import { system_settings } from '../../../../src/server/drizzle/main.js';
...
export const miscRouter = router({
	...
	deleteUser: adminProcedure
		...
		.mutation(async ({ input: { id } }) => {
			const deletedUser = await db.delete(userTable).where(eq(userTable.id, id));
			return deletedUser;
		}),
